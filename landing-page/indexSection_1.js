...
let connection = `
<div class="setting sc-2382db41-3 kZEPZp hIAkVp snipcss-Sz9Q3 snip-div">
...
`
let setting = `<div class=" sc-de7e8801-0 setting sc-2382db41-3 kZEPZp hIAkVp" draggable="false" style="max-width: 420px; user-select: none; touch-action: pan-x;">
    ...
    <div class="sc-de7e8801-1 sc-1080dffc-0 jAfYRQ gcGTlB">
      ...
      <div class="sc-de7e8801-1 sc-1080dffc-0 dUUCVU gcGTlB">
        ...
        <div class="sc-de7e8801-1 sc-1080dffc-0 dUUCVU bTfSja">
          <button class="buttons sc-ee37452c-0 bPhvhB " scale="sm">Standard (5)</button>
          <button class="buttons sc-ee37452c-0 bPhvhB cnIgEn" scale="sm">Fast (6)</button>
          <button class="buttons sc-ee37452c-0 bPhvhB cnIgEn" scale="sm">Instant (7)</button>
        </div>
      </div>
    </div>
  </div>
...
(function settings() {
  ...
  function first(e) {
   ...
   bottoni.forEach((el, index)=> el.addEventListener("click", function() {
    el.classList.add("bPhvhB")
    el.classList.toggle("cnIgEn")
    for (nodo of bottoni){
      if (nodo === el) {continue}
      else if(nodo.classList.contains("cnIgEn")){continue}
      nodo.classList.toggle("cnIgEn")
    }
  ...
  function hide() {
    document.querySelector('.settings > div').addEventListener('click', function (event) {
      event.stopPropagation();
    });
    document.querySelector(`.settings`).style.transition = "opacity .5s";
    document.querySelector(".settings").style.opacity = "0"
...
function darkmode(){document.querySelector('.topSide').addEventListener('click', ()=>{
  if(document.querySelector('body').classList.contains('dark-mode')){
      document.querySelector('body').classList.remove('dark-mode')
      gsap.to('.topSide > div.slider', {x: 15, duration: 0.3})
      document.querySelector('.topSide').style.backgroundColor='#666171'
      document.querySelector('.topSide > div.slider').style.backgroundColor='#27262c'
      document.querySelector('.topSide div.nightMode img').style.filter='invert(33%) sepia(87%) saturate(483%) hue-rotate(219deg) brightness(87%) contrast(86%)'
      document.querySelector('.topSide div.lightMode img').style.filter='invert(12%) sepia(3%) saturate(1947%) hue-rotate(210deg) brightness(94%) contrast(90%)'

  }
  else{
      document.querySelector('body').classList.add('dark-mode')
       gsap.to('.topSide > div.slider', {x: -14, duration: 0.3})
       document.querySelector('.topSide').style.backgroundColor='rgb(189, 194, 196)'
       document.querySelector('.topSide > div.slider').style.backgroundColor='white'
       document.querySelector('.topSide div.nightMode img').style.filter='invert(100%) sepia(5%) saturate(0%) hue-rotate(45deg) brightness(107%) contrast(105%)'
       document.querySelector('.topSide div.lightMode img').style.filter='invert(65%) sepia(67%) saturate(496%) hue-rotate(347deg) brightness(103%) contrast(103%)'
  }
})

document.querySelector('.topSide').addEventListener('click',()=>{
